...
账号级骑士等级不写在站上：`knightLevel = 1 + sum(可玩站 stationLevel - 1)`（等价 `sum(level) - (站数 - 1)`）。站升级时用存档快照补发差额灵感，并写消息箱。旧档缺快照只按当前站等级写入，不把缺字段当成 1 去灌点。工坊入口另按骑士等级开站：采药 1、炼金 2、狩猎 5、烹饪 6、采矿 9、铭刻 10；未开站置灰，点出「骑士 N 级开放站名」。
