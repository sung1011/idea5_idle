...
账号级骑士等级不写在站上：`knightLevel = 1 + sum(可玩站 stationLevel - 1)`（等价 `sum(level) - (站数 - 1)`）。站升级时用存档快照补发差额灵感，并写消息箱。旧档缺快照只按当前站等级写入，不把缺字段当成 1 去灌点。工坊入口另按骑士等级开站：采药 1、炼金 2、狩猎 5、烹饪 8、采矿 18、锻造 20；未开站置灰，点出「骑士 N 级开放站名」。
...
偶遇新单只收当前已解锁工位的产物（采药草/香料起，随骑士开站扩到药剂 / 狩猎 / 烹饪 / 矿 / 锻造工具）。开局商场「铜矿当」仍是 `ore` ×2 特例。新刷不再要通用 `tool` / `ironTool` / `mithrilTool`；旧档武器需求与通用工具库存仍可成交 / 可留。读档时订单消耗 / 报价里的通用工具 remap 成专属工具，库存旧工具 `itemProducerStation` 回落锻造。偶遇板 6 格，每格灰绿蓝紫橙品质（探索不刷灰），高品质需求与产出更高且更赚。
