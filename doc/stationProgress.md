...
偶遇敌人第一期仍收基础 `weapon` / `meal`（及矿、鱼）。高阶铁器需求后做，避免开局卡在未解锁品类。偶遇板仍 5 格，六种订单（敌人 / 黑心商人 / 路人 / 当铺 / 工匠委托 / 收购），每格灰绿蓝紫橙品质（探索不刷灰），高品质需求与产出更高且更赚。

---

## UI

站点卡：等级、生产进度条、XP 条、当前 `costs`、品类按钮。未解锁按钮禁用并标 `LvN`。生产条用显示用进度：权威仍是每秒 `applyTick`，UI 按 `stationSpeed` 在两拍之间插值，停产不假跑；XP 条用短 CSS transition。银行页每个物品有相对 cap 的进度条（≥80% 警告，100% 满仓）。主界面页签：工坊 / 银行 / 工人 / 偶遇。
