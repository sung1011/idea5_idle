...
站点卡：等级、生产进度条、XP 条、当前 `costs`、品类按钮。未解锁按钮禁用并标 `LvN`。生产条用显示用进度：权威仍是每秒 `applyTick`，UI 按 `stationSpeed` 在两拍之间插值，停产不假跑；XP 条用短 CSS transition。工坊顶部「物资」只显示数量与卖出，无容量条。主界面页签：工坊 / 工人 / 偶遇。
