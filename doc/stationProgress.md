...
新物品进 `ITEM_DEF`。制造站卡片只显示当前这次消耗库存（选中品类 `costs` / `altCosts`，炼金当前会走的消耗）；采集无消耗则隐藏该块。产出完成用 `floatTips`「获得」漂字，不常驻产出库存、不存 `recentGains`。主界面不再卖货。换金走偶遇当铺 / 收购。没有容量上限，也不因「满」停产。不做制皮。
...
站点卡：等级、生产进度条、XP 条、当前 `costs`、制造站当前消耗库存、多品类站用下拉选当前生产目标。下拉列出全部已解锁，另加 1 个下一档未解锁（置灰不可选），不列更后面的未解锁。当前品类为选中态；切已解锁即换整站生产目标。生产条用显示用进度：权威仍是每秒 `applyTick`，UI 按 `stationSpeed` 在两拍之间插值，停产不假跑；XP 条用短 CSS transition。吞吐完成时在线 tick 走 `floatTips`「获得」；离线追赶只写消息箱。无独立物资区、无产出库存列表、无卖货按钮、无容量条。主界面页签：工坊 / 工人 / 偶遇。
