...
| 现码 | 定稿 |
| --- | --- |
| 六站 + 伐木主界面 | 七站；伐木废弃 / 藏入口 |
| 无狩猎、无采药 | 新增 `hunting` `herbalism` |
| 锻造出兵器，高阶耗木 | 锻造出工具；武器搁置 |
| 烹饪只耗鱼出 `meal` 可卖 | `meal` 进 `foodSlot` 给生产 Buff |
| 炼金耗木，邻接伐木 / 锻造 | 耗草 / 猎副产，邻接采药 |
| 钓鱼每次必出 1 鱼 | 空杆 + 更慢期望 + 随机 + 品阶墙 |
| Worker 无双槽 | `toolSlot` / `foodSlot` |
| 偶遇补给含武器 / 木头 | 改食物 / 工具 / 矿 |
