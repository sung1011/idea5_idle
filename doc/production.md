...
UI 主列六站。工坊页左侧三组竖签（药剂=采药+炼金，食物=狩猎+烹饪，矿符=采矿+铭刻；组 id 仍是 `weapon`），切组同时显示该组两张站卡；工人页左栏仍按站分行，上→下与工坊组一致：采药→炼金→狩猎→烹饪→采矿→铭刻（`STATION_ORDER` 共用，派入空槽扫描同序）。工人左栏仍按 7 行均分，去掉钓鱼后不把剩余 6 站拉高。`woodcutting` / `fishing` 废弃或藏入口，见第 7 节。
