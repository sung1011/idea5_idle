...
describe('treasure raid hud', () => {
  ...
  it('mounts the shared hp and act bars only on the raid readout', () => {
    ...
    expect(panel).toContain('aria-label="储量"')
    expect(panel).toContain('aria-label="开采进度"')
    expect(panel).toContain('mineDigReadout')
    ...
    expect(panel).not.toContain('开采 {{ names(mine.crewIds) }}')
    expect(panel).not.toContain('无符文）')
